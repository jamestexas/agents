...
| [`design-doc-review`](skills/design-doc-review/SKILL.md) | scoped | Review a design doc for the decision it asks you to ratify — not the prose. |
...
| [`hud`](skills/hud/SKILL.md) | inherits all | Capture a note, playbook, peer, or project-context update into the user's personal work HUD — a local markdown tree at $HUD_ROOT. |
| [`implement`](skills/implement/SKILL.md) | inherits all | Deterministic end-to-end implementation orchestrator. |
| [`jj-stack`](skills/jj-stack/SKILL.md) | scoped | Turn one dev branch into small, individually-reviewable stacked PRs that land on a feature branch (one final merge to main), using jujutsu (jj) colocated with git plus keyless commit signing. |
...
| [`pr-buckets`](skills/pr-buckets/SKILL.md) | scoped | Group a body of in-progress work (a diff, a branch, a set of changes) into small, individually-reviewable PR buckets sized to a LoC threshold — one concern per bucket, in dependency order. |
...
| [`pr-suggestion`](skills/pr-suggestion/SKILL.md) | scoped | Post a GitHub review comment as an applyable "suggested change" — a ```suggestion block the author commits with one click — and, first, decide whether a finding should be a suggestion at all. |
