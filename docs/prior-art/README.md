...
| Axis | [Smithy](smithy.md) | [Buf](buf.md) | [SPIFFE](spiffe.md) | [Macaroons](macaroons.md) | [Nixpkgs](nixpkgs.md) | [SLSA+Sigstore+in-toto](slsa-sigstore-in-toto.md) | [WIT](wit.md) | [Backstage](backstage.md) `[planned]` | **Us** ([baseline](_baseline.md)) |
|---|---|---|---|---|---|---|---|---|---|
| 1. IDL shape | `.smithy` IDL, services + shapes | protobuf only; operates over `.proto` | SPIFFE ID URI + SVID formats | credential format, not IDL | not an IDL — package collection in Nix language | in-toto Statement + DSSE envelope | wasm-scoped IDL; types + interfaces + worlds | YAML templates | capnp |
| 2. Annotation / trait model | **rich** — typed traits, custom-declarable, propagate through codegen | inherits protobuf custom options; no Buf trait library | none — SVIDs are leaves | caveats as application-defined predicates | out of scope (`meta` attrs, not propagating) | predicate-type URIs (flat, interoperable) | versioning gates only: `@since`/`@unstable`/`@deprecated` | template parameters | informal capnp `$annotation` |
| 3. Versioning + breaking-change | `smithy diff` + projections | `buf breaking --against`; FILE/PACKAGE/WIRE tiers | bundle `spiffe_sequence` rotation | n/a | bi-annual tested channels + `flake.lock` (narHash) | per-spec semver, no diff tool | semver packages + encoded gates; no diff tool | template versions | per-dir, no automation |
| 4. Codegen targets + plugin model | plugin-driven; Java/Kotlin/TS/Python/OpenAPI | `buf.gen.yaml`; remote/local/builtin plugins | reference SDKs + attestor plugins (SPIRE) | n/a | multi-platform builds via Hydra (not IDL codegen) | out of scope (build-system integrations) | wit-bindgen: Rust/C/C++/C#/Go/MoonBit (guest, pre-stable) | scaffold-time only | single-target (zod), hardcoded |
| 5. Identity / capability model | out of scope (auth-as-trait descriptor) | out of scope | **canonical workload identity** — SVID + trust domain | **canonical capability tokens** — HMAC chain, attenuation, discharge | static substituter keys (no workload identity) | Fulcio OIDC→X.509 + Rekor witness (signet already implements) | out of scope; `resource` is object-cap handle | n/a | interlace lease (macaroon-shaped) + signet (SPIRE/Fulcio-shaped) |
| 6. Supply-chain story | out of scope | content-addressed commits + lockfile digest only | out of scope | out of scope | **strong** — reproducible-by-default + CycloneDX via bombon/sbomnix | **canonical** — SLSA L1/L2/L3 + signed in-toto attestations + Rekor log | out of scope | n/a | primitives exist (signet+cosign bridge), nothing wired into releases |
| 7. Adoption cost | M (patterns) / L (wholesale) | S (patterns) / L (wholesale: protobuf migration) | L (SPIRE deploy) / S (vocabulary) | S (we mostly have it) | S (patterns: lockfile+channel) / L (wholesale: Nix paradigm) | S (L1+sign+Rekor) / L (L3) | S (patterns: gates+world) / L (wholesale: wasm-coupled) | M (Backstage runtime) | n/a — we're the baseline |
...
| Nixpkgs | No | Lockfile shape (`flake.lock` → `art.lock`); bi-annual coordinated channels (`art-2026.05.0`); content-addressed pinning (narHash analog via ley-line CIDs); CycloneDX SBOM generation per release | Wholesale Nix adoption is a quarter-of-work paradigm shift; no Nix expertise on the team; static `trusted-public-keys` is the SPIRE-anti-pattern signet exists to replace |
| WIT + Component Model | No | `@since`/`@deprecated`/`@unstable` gate pattern as capnp annotations; `world` abstraction as `$World(imports, exports)`; plugin-driven codegen architecture (confirms Smithy/Buf recommendation) | WIT is wasm-component-coupled; wit-bindgen is explicitly pre-stable; capnp is broader. Treat as future complement when cloister ships wasm components — watch quarterly |
...
2. **Backstage software templates** — for the recipe / scaffolding side.
3. **CUE / Pkl** — if schema-bridge ever needs to render configs (not just types).
4. **TypeSpec** (Microsoft) — adjacent to Smithy; lower-priority duplicate.
5. **Biscuit tokens** — asymmetric-signature macaroon variant; surfaced as a follow-up by the Macaroons entry (signet's construction may already align with Biscuit). `[unverified]` until a session evaluates it.
