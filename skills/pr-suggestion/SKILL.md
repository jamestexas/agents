...
## What the comment around the fence says

A suggestion ships with prose above the fence, and that prose is the review. The
fence says *what* to change; the sentence above it is the only place the author
learns whether they have to, and why:
...
The severity tag matters more here than anywhere else, because a one-click
"Commit suggestion" button reads as an instruction whatever the words say. A
`[Non-blocking]` suggestion the author declines should cost them one click of
"dismiss," not a justification.
