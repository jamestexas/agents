...
Nothing here is posted, but each finding inside the teaching is still written as
if it were — tagged, cited, and framed the same way — so the user can lift one
into a review comment without rewriting it, and so a `[Question]` never reads as
a verdict just because it was never sent:
